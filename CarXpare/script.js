//script.js for data pharsing
//car_data to be obtained via database(car_data in json format)
var car_data = {
	"i20" : {
		"Name" : "Hyundai i20",
...
//immediate invoke function(IIF) to add the data "Name" field in both dropdowns
(function set_drops() {
	let temp = '';
	...
	drop[0].innerHTML = drop_start + temp; 		//for first dropdown
	drop[1].innerHTML = drop_start + temp; 		//for second dropdown

})();

//"click" event listener for compare button
compare_btn.addEventListener('click', () => {

	//defining compare_car1 and 2 for determing selected option by user
	...
	if (compare_car1 === compare_car2 || (compare_car2 == "-1" || compare_car1 == "-1")) {
		//for same option selection at both dropdowns or no selection at any one dropdown

		error_element.innerHTML = error_text;
	}
	else {
		//if selected option are different, car_data pharsing is started.
		...
		for (const parameter in car_data[compare_car1]) {
			if (parameter === "Name") {
				//Pharsing the car_data for "Name" in the header of table
				...
				//Pharsing car_data for image/preview display

				if (screen.width < 992) continue;		//no previw for sm devices dur to misalignment of content
				else {
				

				datahtml.innerHTML += `<tr><td class="fw-bold">`+parameter+`</td>
				...
				//Pharsing car_data for remaining parameters
